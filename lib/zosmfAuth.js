  ...
  getStatus(request, sessionState) {
    return new Promise((resolve, reject) => {
      this._authenticateOrRefresh(request, sessionState, true).then((response) => {
        //TODO: if we knew whether or not zosmf was renewing sessions, then we could tick down expms from last call
        resolve(response);
      }).catch ((e)=> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.zluxData.plugin.callService("zosmf", loginUrl, options).then(response => {
        ...
        if (zosmfCookie) {
          ...
          sessionState.zosmfSessionEstablishedms = Date.now();
